...
public class SpawnManager : MonoBehaviour
{
    ...
    public void SpawnBullet(Transform enemyTransform=null, Bullet enemyBullet=null)
    {
        ...
        {
            if (!enemyTransform.gameObject.GetComponent<Hunter>()._haveBullet)
            {
                ...
                else
                {
                    ...
                    var bulletSctript = bullet.GetComponent<Bullet>();
                    bulletSctript.PutMe.Subscribe(PutObject).AddTo(_disposables);
                }
                bullet.transform.parent = transform;
                var position = enemyTransform.transform.position;
                bullet.transform.position = new Vector3(position.x, position.y - 1.2f, 0);
                enemyTransform.gameObject.GetComponent<Hunter>()._haveBullet = true;
