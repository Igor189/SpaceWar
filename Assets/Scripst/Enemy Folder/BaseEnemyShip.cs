...
public abstract class BaseEnemyShip : MonoBehaviour
{
    ...
    [HideInInspector] public Vector3 _playerLastPos=Vector3.zero;
    ...
    {
        var random = UnityEngine.Random.Range(0, 100);
        if (random < Controller.Instance._procentBonusMoney)
        {
            Instantiate(Controller.Instance._moneyBonusPref, transform.position, new Quaternion(0, 0, 0, 0));
        }
    }    
}
